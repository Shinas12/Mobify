  ...
  if(req.session.userLoggedIn){
    ...
    LoginHelper.getcartcount(req.session.user._id).then((number)=>{ 
      LoginHelper.getProductnfofull().then((products)=>{ 
        ...
        res.render('user/index-3',{Header:true,homePage:true,userLoggedIn,number,category,cathover:true,sPro,products,CartItems,user})
    ...
    LoginHelper.addCategory().then((category)=>{
      LoginHelper.getProductnfofull().then((products)=>{
        ...
        res.render('user/index-3',{Header:true,homePage:true,userLoggedIn,category,cathover:true,sPro,products,CartItems})
...
router.get('/category',paginatedResults,GotoCart,findAllWishlist,function(req, res, next) {
  ...
  let user = req.session.user._id
    ...
    LoginHelper.getcartcount(user).then((number)=>{
      LoginHelper.addCategory().then((category)=>{    
        res.render('user/category',{Header:true,homePage:false,userLoggedIn,products,number,category,next,previous,pages,pageCount,currentPage,CartItems,user,Wish})
      })
  ...
  let Wish = res.Wishlist
      ...
      LoginHelper.addCategory().then((category)=>{
    res.render('user/category',{Header:true,homePage:false,userLoggedIn,products,category,next,previous,pages,pageCount,currentPage,CartItems,Wish})
      })
  ...
  LoginHelper.ViewCart(req.session.user._id).then(async (product)=>{
    ...
    if (totalValue[0]){
      ...
      LoginHelper.ViewCartDiscount(req.session.user._id,total).then((discount)=>{
        if(discount[0]){
          res.render('user/cart',{Header:true,homePage:false,userLoggedIn,product,total,discount})
        }else{
          res.render('user/cart',{Header:true,homePage:false,userLoggedIn,product,total})
        }
        
      })
      
    }else{
      res.render('user/cart',{Header:true,homePage:false,userLoggedIn,product})
    }
              ...
              if(discount[0].TotalAfterDiscount){
                ...
                LoginHelper.getAddress(req.session.user._id).then((adrs)=>{
                  LoginHelper.getMoreAddress(req.session.user._id).then((adrsMore)=>{
                    res.render('user/checkout',{Header:true,homePage:false,userLoggedIn,product,discount,total,user:req.session.user,adrs,AcaStyle:true,adrsMore})
                  })
                ...
                LoginHelper.getAddress(req.session.user._id).then((adrs)=>{
                  LoginHelper.getMoreAddress(req.session.user._id).then((adrsMore)=>{
                    res.render('user/checkout',{Header:true,homePage:false,userLoggedIn,product,total,user:req.session.user,adrs,AcaStyle:true,adrsMore})
                  })
   ...
   LoginHelper.OrderSummaryDetails(req.session.user._id).then((orderDetails)=>{
    LoginHelper.OrderSummaryDetailsGrandTotal(req.session.user._id).then((ordergrandTotal)=>{
      ...
      if(ordergrandTotal[0]){
        ...
        res.render('user/order',{Header:true,homePage:false,userLoggedIn,orderDetails,OgrandTot})
      }else{
        res.render('user/order',{Header:true,homePage:false,userLoggedIn,orderDetails})
      }
...
router.get('/returnProductagain',verify,(req,res)=>{
  LoginHelper.specificproductdetails(req.session.return).then((orderdetails)=>{
    ...
    res.render('user/returnform',{Header:true,homePage:false,userLoggedIn,orderdetails})
  })
})
...
router.get('/myaccount',verify,(req,res)=>{
  ...
  res.render('user/myaccount',{Header:true,homePage:false,userLoggedIn})
})
//=====================================================================================================================
...
router.get('/changepassword',verify,(req,res)=>{
  res.render('user/changepassword',{Header:true,homePage:false,userLoggedIn,errMessage})
  errMessage = ""
})
...
router.get('/userEdit',verify,(req,res)=>{
  LoginHelper.getUserInfoforEdit(req.session.user._id).then((user)=>{
    console.log(user)
    res.render('user/userEdit',{Header:true,homePage:false,userLoggedIn,errMessage,user})
    errMessage=""
  })
...
router.get('/addressadd',(req,res)=>{
  res.render('user/addressadd',{Header:true,homePage:false,userLoggedIn})
})
...
router.get('/addressManagement',verify,(req,res)=>{
  ...
  res.render('user/addressadd',{Header:true,homePage:false,userLoggedIn,adrs,AddrssStyle:true})
  })
})
...
router.get('/EditAddressPages',verify,(req,res)=>{
  LoginHelper.addressfind(req.session.addressid).then((addrss)=>{
    ...
    res.render('user/EditaddressPage',{Header:true,homePage:false,userLoggedIn,addrss})
   })
})
        ...
        LoginHelper.addCategory().then((category)=>{    
          LoginHelper.filteredProducts(req.body).then((filteredProduct)=>{
            if(filteredProduct == true){
              ...
              res.render('user/filteredProduct',{Header:true,homePage:false,userLoggedIn,number,category,CartItems,user,Wish,filteredProduct})
            }
        ...
        LoginHelper.addCategory().then((category)=>{
          LoginHelper.filteredProducts(req.body).then((filteredProduct)=>{
            if(filteredProduct == true){
              ...
              res.render('user/filteredProduct',{Header:true,homePage:false,userLoggedIn,category,CartItems,Wish,filteredProduct})
            }
